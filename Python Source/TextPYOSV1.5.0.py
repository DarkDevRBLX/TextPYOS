...
def ColoramaSettings():
    global ColoramaEnabledNum
    ColoramaChange = input ("Colorama On/Off: ").strip().lower()
    if ColoramaChange == "on":
        ColoramaEnabledNum = 1
        DirectoryFUNC()
    elif ColoramaChange == "off":
        ColoramaEnabledNum = 0
        DirectoryFUNC()
    else:
        print ("Error, please say 'on' or 'off'")
        ColoramaSettings()


def ClockFUNC():
    #Sorry this is so simple, I'll add more later.
...
#Software Variables
def DirectoryFUNC():
    Commands = ["-usercode", "-help", "commands", "-calculator", "-AccountSettings", "-Colorama", "-Clock", "-exit"]
    ...
    if Directory == "-commands":
        ...
        CalculatorApp()
    elif Directory == "-colorama":
        ColoramaSettings()
    elif Directory == "-clock":
        ClockFUNC()
    elif Directory == "-exit":
        quit()
    else:
